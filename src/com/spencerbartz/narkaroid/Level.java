...
public class Level {
	...
	public Level(Image bg, Image startImg, Image contImg, BlockHolder blockHolder, int introDelay, int outroDelay,
			int contDelay) {
		this.bg = bg;
		...
		BufferedImage clearImage = imageLoader.loadImage("images/levelclear.gif");
		Level.outroImage = clearImage;

		// load image for paused
		BufferedImage pausedImage = imageLoader.loadImage("images/paused.gif");
	...
	public void intro(Graphics g) {
		...
		if (introDelay == 0) {
			...
			// System.out.println("LEVEL " + parent.getCurrentLevelNum() + " INTRO
			// FINISHED");
		}
	}
	...
	public void cont(Graphics g) {
		...
		if (contDelay == 0) {
			...
			// when cont = false, stop showing 'Level N' graphic on screen, gameplay can
			// resume
			cont = false;
	...
	public void outro(Graphics g, LevelManager parent) {
		...
		if (outroDelay == 0) {
			if (parent.allPowerUpsOffScreen()) {
				...
				if (parent.getCurrentLevelNum() < LevelManager.NUM_LEVELS - 1)
