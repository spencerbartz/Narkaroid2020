...
//	private static final int LVL1_PWRUPS = 2;
	...
	public LevelManager() {
		...
		for (int i = 0; i < bgImages.length; i++) {
			bgImages[i] = imageLoader.loadImage("images/bg" + i + ".jpg");
			startImages[i] = imageLoader.loadImage("images/start" + i + ".gif");
			contImages[i] = imageLoader.loadImage("images/cont" + i + ".gif");
		}
		...
		/*
		 * //level 1 contains a custom block holder Block [][] level1Blocks = new
		 * Block[LVL1_COLS][LVL1_ROWS]; int lvl1InitialX = (RenderingPanel.SCREEN_WIDTH
		 * / 2) - ((LVL1_COLS / 2) * Block.DEFAULT_WIDTH); int lvl1InitialY = 40;
		 * 
		 * for(int i = 0; i < LVL1_COLS; i++) for(int j = 0; j < 2; j++)
		 * level1Blocks[i][j] = new Block(Block.RED_BLOCK);
		 * 
		 * for(int i = 0; i < LVL1_COLS; i++) for(int j = 1; j < LVL1_ROWS; j++)
		 * level1Blocks[i][j] = new Block(Block.RED_BLOCK);
		 * 
		 * BlockHolder level1 = new BlockHolder(LVL1_COLS, LVL1_ROWS, level1Blocks,
		 * lvl1InitialX, lvl1InitialY); levels[1] = new Level(bgImages[1],
		 * startImages[1], contImages[1], level1);
		 * 
		 * //level 2 contains a hand crafted custom block holder Block [][] level2Blocks
		 * = { { new Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), new
		 * Block(Block.GREEN_BLOCK), new Block(Block.GREEN_BLOCK), new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.GREEN_BLOCK), }, { new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), new
		 * Block(Block.GREEN_BLOCK), new Block(Block.GREEN_BLOCK), new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), }, { new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), new
		 * Block(Block.GREEN_BLOCK), new Block(Block.GREEN_BLOCK), new
		 ...
		 * Block(Block.GREEN_BLOCK), new Block(Block.GREEN_BLOCK), new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), }, { new
		 * Block(Block.RED_BLOCK), new Block(Block.RED_BLOCK), new
		 * Block(Block.RED_BLOCK), new Block(Block.RED_BLOCK), new
		 * Block(Block.RED_BLOCK), new Block(Block.RED_BLOCK), }, { new
		 * Block(Block.RED_BLOCK), new Block(Block.RED_BLOCK), new
		 * Block(Block.RED_BLOCK), new Block(Block.RED_BLOCK), new
		 ...
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), new
		 * Block(Block.GREEN_BLOCK), new Block(Block.GREEN_BLOCK), new
		 * Block(Block.YELLOW_BLOCK), new Block(Block.YELLOW_BLOCK), }, };
		 * 
		 * 
		 * 
		 * //LVL2 initial coordinates for block holder int lvl2InitialX =
		 * (RenderingPanel.SCREEN_WIDTH / 2) - ((LVL2_COLS / 2) * Block.DEFAULT_WIDTH);
		 * int lvl2InitialY = 40;
		 * 
		 * BlockHolder level2 = new BlockHolder(LVL2_COLS, LVL2_ROWS, level2Blocks,
		 * lvl2InitialX, lvl2InitialY); levels[2] = new Level(bgImages[2],
		 * startImages[2], contImages[2], level2);
		 */
		// create power ups for levels
		// power ups stored in double dimension array. first index corresponds to level
		// number
		...
		/*
		 * //lvl1 powerUps[1] = new PowerUp[LVL1_PWRUPS]; powerUps[1][0] = new
		 * PowerUp(PowerUp.LASER); powerUps[1][1] = new PowerUp(PowerUp.LASER);
		 * 
		 * //lvl2 powerUps[2] = new PowerUp[LVL2_PWRUPS]; powerUps[2][0] = new
		 * PowerUp(PowerUp.SPEEDUP); powerUps[2][1] = new PowerUp(PowerUp.EXTRA_LIFE);
		 * powerUps[2][2] = new PowerUp(PowerUp.EXTRA_LIFE);
		 */
	 ...
	 * continueLevel()
	 * @param g
	 * This resumes a level which has been paused (I think)
	 */
	...
	public void printLevelState() {
		System.out.println("Started: " + levels[currentLevel].isStarted() + " cleared: "
				+ levels[currentLevel].isCleared() + " finished: " + levels[currentLevel].isFinished() + " continuing: "
				+ levels[currentLevel].isContinuing());
	}
